...
> **순수 HTML/CSS/JS + Vercel Serverless Functions + MongoDB Atlas**
> 로 전면 변경했습니다. 아래 내용은 모두 최신 스택 기준입니다.
> (이미지 첨부 기능은 범위에서 제외하고, 텍스트 게시판 CRUD에 집중합니다.)
...
브라우저 → HTML/CSS/JS 페이지 → Serverless Functions → MongoDB
순서로 요청이 흐르고, 결과는 반대 방향으로 되돌아옵니다. 페이지와 API 함수는
둘 다 Vercel 위에서 무료로 함께 호스팅됩니다.

---

# 6. CRUD 흐름 완전 정리 (핵심 학습 파트)
...
1. 비밀번호를 그대로 보내지 않고 해시로 변환합니다.
2. `POST /api/posts`로 닉네임/비밀번호해시/제목/내용을 전송합니다.
3. API 함수가 필수값을 검사하고 `insertOne()`으로 DB에 새 문서를 추가합니다.
4. 성공하면 새로 생긴 글 정보와 함께 201 응답, 페이지는 목록으로 이동합니다.

---

## 6.2 Read — 조회하기 (목록 / 상세, 2가지가 있음)

### 6.2-A 목록 조회 (페이지네이션 + 검색 포함)
...
(6.3, 6.4에서 실제 사용 예시 참고).

## 8. 페이지네이션 개념

![페이지네이션 개념](images/pagination.svg)

화면 하단 `[1] [2] [3]` 버튼 클릭 → `GET /api/posts?page=번호` 재요청 (6.2-A 참고)

---

*배포까지 완료되어 실제로 동작하는 상태입니다. 전체 CRUD(작성/조회/수정/삭제/댓글/검색/페이지네이션)를
Vercel에 배포된 사이트에서 직접 테스트하며 확인했습니다.*
